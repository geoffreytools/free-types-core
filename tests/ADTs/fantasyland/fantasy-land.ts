...
import { Type, apply, HKT, A } from '../../../src/'
...
test('Maybe.fromNullable infers the narrowest type possible', t => [
...
test('Maybe.map works on monomorphic fns', t => [
...
test('Either.ap works on monomorphic fns', t => [
    t.equal (eitherNum.ap(eitherExclaim)) <Either<CustomError, string>>(),
]);

test('Either.ap works on directly applied polymorphic fns', t => [
    t.equal (eitherNum.ap(either<CustomError>()(tuple)), <Either<CustomError, [number]>>_),
]);

test('Either.ap works on all polymorphic fns', t => [
    // @ts-expect-error: design limitation
